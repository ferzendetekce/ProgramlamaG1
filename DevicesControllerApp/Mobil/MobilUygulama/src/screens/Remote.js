...
// 1. ADIM: Gerekli bildirim fonksiyonlarını import ediyoruz
import { 
  scheduleTherapyInProgressNotification, 
...
const Remote = ({ navigation }) => {
  ...
  const sendCommand = async (command) => {
    if (host) {
      // Hata yakalama bloğu ekleyerek API hatalarında bildirim gönderelim
      try {
        await remoteService.remoteService(host.ip, host.port, command);
      } catch (error) {
        console.error("Komut gönderilirken hata oluştu:", error);
        // Hata durumunda bildirim gönderiyoruz
      ...
      <ControlSection title="Terapi Kontrolleri">
        <View className="flex-row flex-wrap justify-center">
          <ControlButton 
            onPress={() => {
              sendCommand("start");
              // 2. ADIM: Başlat butonuna basınca bildirimi gönder
              scheduleTherapyInProgressNotification(); 
            }} 
            text="Başlat" 
          ...
          <ControlButton 
            onPress={() => {
              sendCommand("stop");
              // 3. ADIM: Durdur butonuna basınca bildirimleri iptal et
              cancelAllNotifications();
            }} 
            text="Durdur" 
          ...
          <ControlButton 
            onPress={() => {
              ...
              // 4. ADIM: Acil Stop'a basınca bildirimleri iptal et
              cancelAllNotifications();
            }} 
            text="Acil Stop" 
